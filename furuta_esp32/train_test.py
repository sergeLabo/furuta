...
class TrainTest:
    ...
    def __init__(self, current_dir, config_obj, numero):
        ...
        if not os.path.exists(my_models_dir):
            ...
            print(f"Création de {my_models_dir}")
        if not os.path.exists(models_dir):
            os.makedirs(models_dir)
            print(f"Création de {models_dir}")
    ...
    def training_tensorboard(self):
        ...
        dt_now = datetime.now()
        dt = dt_now.strftime("%d-%m-%Y | %H:%M")
        # Temps en secondes du train complet.  periode = 35 ms 28 Hz
